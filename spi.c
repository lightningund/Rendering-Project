...
void spiInit() {
	...
	WAIT_FOR_NOT_VAL(GCLK_REGS->GCLK_SYNCBUSY, GCLK_SYNCBUSY_GENCTRL_GCLK3);
	...
	WAIT_FOR_VAL(GCLK_REGS->GCLK_PCHCTRL[8], GCLK_PCHCTRL_CHEN_Msk);
	...
	PORT_REGS->GROUP[0].PORT_PMUX[16/2] |= PORT_PMUX_PMUXE_C;
	PORT_REGS->GROUP[0].PORT_PINCFG[17] |= PORT_PINCFG_PMUXEN_Msk;
	PORT_REGS->GROUP[0].PORT_PMUX[17/2] |= PORT_PMUX_PMUXO_C;
	PORT_REGS->GROUP[0].PORT_PINCFG[19] |= PORT_PINCFG_PMUXEN_Msk | PORT_PINCFG_INEN_Msk;
	PORT_REGS->GROUP[0].PORT_PMUX[19/2] |= PORT_PMUX_PMUXO_C;
}
...
void spiFlushBuffer() {
	...
	while ((SERCOM1_REGS->SPIM.SERCOM_INTFLAG & SERCOM_SPIM_INTFLAG_RXC_Msk) == SERCOM_SPIM_INTFLAG_RXC_Msk) {
		dummy = SERCOM1_REGS->SPIM.SERCOM_DATA;
	}

	(void)dummy; // hack to get rid of the unused var warning
}
...
uint8_t spiReadByte() {
	...
	WAIT_FOR_VAL(SERCOM1_REGS->SPIM.SERCOM_INTFLAG, SERCOM_SPIM_INTFLAG_RXC_Msk);
