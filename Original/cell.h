...
#define CELL_GRID_DIM_X 8
#define CELL_GRID_DIM_Y (CELL_GRID_DIM_X * 2)
...
static inline GridCell cell_wall_section(const GridCell wall, const idx_t dir) {
	return (wall >> (CellWallBits * dir)) & 0xF;
}

// Gets the texture bits of the wall section
static inline uint8_t cell_wall_texture(const GridCell wall, const idx_t dir) {
	return cell_wall_section(wall, dir) >> 2;
}

// Tests the fake bit of the wall section
static inline bool cell_wall_is_fake(const GridCell wall, const idx_t dir) {
	return (cell_wall_section(wall, dir) & WALL_FAKE_BIT) != 0;
}

// Tests the "there" bit of the wall section
static inline bool cell_wall_is_there(const GridCell wall, const idx_t dir) {
	return (cell_wall_section(wall, dir) & WALL_THERE_BIT) != 0;
}

// Sets the fake bit of the wall section
static inline void make_cell_wall_fake(GridCell* wall, const idx_t dir) {
	*wall |= WALL_FAKE_BIT << (CellWallBits * dir);
}

// Unsets the fake bit of the wall section
static inline void make_cell_wall_real(GridCell* wall, const idx_t dir) {
	*wall &= (GridCell)(-1) ^ (WALL_FAKE_BIT << (CellWallBits * dir));
}

// Sets the "there" bit of the wall section
static inline void make_cell_wall_there(GridCell* wall, const idx_t dir) {
	*wall |= WALL_THERE_BIT << (CellWallBits * dir);
}

// Unsets the "there" bit of the wall section
static inline void make_cell_wall_gone(GridCell* wall, const idx_t dir) {
	*wall &= (GridCell)(-1) ^ (WALL_THERE_BIT << (CellWallBits * dir));
}

static inline void set_cell_wall_texture(GridCell* wall, const idx_t dir, TexID tex) {
...
static inline bool inside_cell(const Cell c, const Vec p) {
	return
		((c.x + 1) * CELL_SIZE) > p.x &&
		(c.x * CELL_SIZE) < p.x &&
		((c.y + 1) * CELL_SIZE) > p.y &&
